...
| 0.1.3 | 2026-04-28 | 附录 C 对齐：管理端 **预约信息**（`leads`）全量 CRUD + `PATCH` 快捷改状态；UI 侧栏「预约信息 / 文章管理」命名；文章列表支持行内删除 |
| 0.1.4 | 2026-05-12 | 附录 C：**艺考咨询助手**与**知识库管理**端点落地（RAG + PDF/Word）；细节见 [implementations/艺考咨询助手.md](./implementations/艺考咨询助手.md) |
| 0.1.5 | 2026-05-14 | 附录 C 补齐文章 AI 流水线与健康检查响应约定；与 README、开发方案 §12 同步 |
...
| `POST /api/v1/leads` | **已实现** | 写入 `leads`；含 `status` 默认 `new`；**应用层**按 IP 限流（`LEADS_RATE_LIMIT`，见 `.env.example`）；人机验证/网关 WAF 待加 |
| `POST /api/v1/auth/login` 或等价管理登录 | **已实现** | JWT，环境变量配置管理员账号与 bcrypt 哈希；见仓库 `.env.example` |
| 管理预约信息 `/api/v1/admin/leads` | **已实现** | `GET` 分页列表；`POST` 新建（后台录入）；`GET/{id}` 详情；`PUT/{id}` 更新字段（至少一项）；`PATCH/{id}` 仅更新 `status`（列表快捷操作）；`DELETE/{id}` 删除。产品侧仍称「线索/留资」，管理端 UI 称 **预约信息** |
| 管理文章 `/api/v1/admin/articles` | **已实现** | 创建/读/改/删；`published_at` 为空为草稿、公开列表不展示；管理端 UI 称 **文章管理**，列表支持行内删除 |
| 管理文章 AI 流水线 `/api/v1/admin/articles/{id}/pipeline/*` 与修订 `/revisions*` | **已实现** | `outline`、`body`、`excerpt` 为 SSE 流式生成；`GET /revisions` 查看修订，`POST /revisions/{revisionId}/apply` 应用修订，`DELETE /revisions/{revisionId}` 删除。细节见 [文章生产流水线-实现说明.md](./文章生产流水线-实现说明.md) |
| `GET /health/live`、`/health/ready` | **已实现** | 不属于 `/api/v1`；面向探活/负载均衡，响应为 `status`/`database` 形态，不使用业务封套 |
...
| `POST /api/v1/assistant/chat`（SSE） | **已实现** | 「艺考咨询助手」：公开流式对话，依据知识库混合检索（稠密向量 + BM25 + 可选重排）；限流 `ASSISTANT_RATE_LIMIT`。说明见 [implementations/艺考咨询助手.md](./implementations/艺考咨询助手.md) |
| 管理知识库 `/api/v1/admin/kb/documents` 等 | **已实现** | PDF/Word 入库、分块、embedding、`draft`/`approved`；见同上 |

**前端与上述接口的对应关系**：`lib/api-server` 在服务端拉取 `site/config`、`articles`、单篇 by slug；`LeadForm` `POST` `leads`；**Next 管理端** `/admin/*` 使用 `lib/adminApi` 对接上述管理 API（侧栏 **预约信息**、**文章管理**、**知识库** `/admin/kb` 等）；预约：`/admin/leads`、`/admin/leads/new`、`/admin/leads/[id]`；文章：`/admin/articles`、`/admin/articles/new`、`/admin/articles/[id]`；资讯列表链至 `/news/[slug]`，正文以 Markdown 安全渲染（见开发方案 §12.2）。官网 `(site)/layout` 挂载 `ConsultAssistant` 咨询浮层。

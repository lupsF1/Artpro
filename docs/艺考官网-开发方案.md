...
- **入口**：`app/main.py` — 注册 `CORSMiddleware`、校验异常处理、`APIError`/HTTP 异常/未捕获异常统一 JSON 包、`/api/v1` 子路由、OpenAPI 路径 `/docs`、`/redoc`、`/openapi.json`。
- **v1 路由**（`app/api/v1/router.py` 聚合）：
  ...
  - `admin`（`app/api/v1/admin.py`，前缀 `/admin`，**须 Bearer**）：**预约信息** `GET/POST /admin/leads`，`GET/PUT/PATCH/DELETE /admin/leads/{id}`；`PUT /admin/site/config`；**文章** `GET/POST /admin/articles` 与 `GET/PUT/DELETE /admin/articles/{id}`；文章 AI 流水线 `POST /admin/articles/{id}/pipeline/outline|body|excerpt`（SSE）与修订 `GET /admin/articles/{id}/revisions`、`POST /admin/articles/{id}/revisions/{revisionId}/apply`、`DELETE /admin/articles/{id}/revisions/{revisionId}`，详见 [文章生产流水线-实现说明.md](./文章生产流水线-实现说明.md)。
  - **`admin/kb`（知识库，须 Bearer）**：`POST /admin/kb/documents`（multipart 上传 PDF/Word）、`GET /admin/kb/documents` 分页、`PATCH /admin/kb/documents/{id}`（`review_status`：`draft`/`approved`）、`DELETE /admin/kb/documents/{id}`。实现见 `app/api/v1/kb_admin.py`；模型 `KbDocument`、`KbChunk`（向量存 `embedding_json`，检索侧在应用层混排：稠密相似度 + BM25 + 可选 rerank）。配置：`EMBEDDING_*`、`KB_*`、`RAG_*`、`RERANKER_*`、`ASSISTANT_RATE_LIMIT`，见 `.env.example`。分支说明：[implementations/艺考咨询助手.md](./implementations/艺考咨询助手.md)。
  - `assistant`（公开）：`POST /assistant/chat` — JSON `messages`，**SSE**（`delta`/`done`/`error`），限流 `ASSISTANT_RATE_LIMIT`。实现见 `app/api/v1/assistant.py`；RAG 见 `app/services/kb_retrieval.py`、`consult_assistant.py`。
...
- **健康**：`GET /health/live`、`/health/ready`（连库检查）；用于探活与负载均衡，响应为 `status`/`database` 形态，不使用业务封套。
- **横切**：`app/core/responses.py` 提供 `ok` / `err`；`app/core/api_errors.py` 提供 `APIError`；`app/deps.py` 含 `get_current_admin`（JWT）；`422` 校验错误、HTTP 异常与未捕获异常统一 JSON 封套；DB session 在异常路径回滚。
- **数据**：`app/db.py`；`app/models/`：`lead.py`、`site_config.py`、`article.py`、`kb_document.py`、`kb_chunk.py`；**Alembic** 迁移链 `0001`→`0006`。默认 SQLite 于 `data/artpro.db`；可选用 **PostgreSQL**（根目录 `.env` 中 `DATABASE_URL` / `DATABASE_URL_SYNC`）与 [docker-compose.yml](../docker-compose.yml)、[scripts/postgres-docker-migrate.sh](../scripts/postgres-docker-migrate.sh)，见根 [README.md](../README.md)。
...
- **管理端 UI**：`app/(admin)/admin/`，`AdminAuthGate` / `AdminShell` / `AdminNav`（侧栏 **预约信息**、**站点**、**文章管理**、**知识库**）。预约：`/admin/leads`（列表、快捷 `PATCH` 状态、行内删）、`/admin/leads/new`、`/admin/leads/[id]`（表单 + 删）。文章：`/admin/articles`（列表行内删）、`/admin/articles/new`、`/admin/articles/[id]`；编辑含 `AdminArticleMdPreview`。知识库：`/admin/kb`（上传 PDF/Word、列表、`draft`/`approved`、删除）。
- **艺考咨询助手（官网）**：`(site)/layout.tsx` 挂载 `ConsultAssistant`；`lib/consultStream.ts` 请求 `POST /api/v1/assistant/chat`（SSE）。
...
- 在新功能分支上开发时：除更新本节（§12）、PRD 附录 C 等**已有文档**外，应在 [`docs/implementations/`](../implementations/) 下新增 **`{分支名}.md`**，记录该分支的范围、接口、环境变量、迁移与联调步骤，便于回顾与交接。
- 当前示例：[艺考咨询助手.md](./implementations/艺考咨询助手.md)（知识库 RAG + 文档入库 + 公开咨询 SSE）。
...
当前在根目录维护 [`openapi/openapi.json`](../openapi/openapi.json)，由 `python3 scripts/export-openapi.py --write` 从 FastAPI 运行态 schema 导出；CI 使用 `--check` 防止代码变更后合同快照未同步。短期以前端手写调用为主，后续可基于该文件接入 `openapi-typescript` 或同类工具生成 `apps/web` 的接口类型。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 0.1.7 | 2026-05-14 | §12：艺考咨询助手知识库检索升级为 PostgreSQL + pgvector，SQLite 保留 JSON fallback |
| 0.1.6 | 2026-05-14 | §12：补齐文章 AI 流水线端点、健康检查响应约定；落地版本化 `openapi/openapi.json` 与 CI 校验 |
| 0.1.5 | 2026-05-12 | §12：**艺考咨询助手**与**知识库**代码落地（`assistant/chat` SSE、`admin/kb`、前端 `/admin/kb` 与 `ConsultAssistant`）；§12.3.1 分支实现文档约定 |

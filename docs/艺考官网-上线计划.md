...
- **限流**：应用层已对 `POST /api/v1/leads` 按 IP 限流（[`apps/api/app/api/v1/leads.py`](../apps/api/app/api/v1/leads.py) + slowapi）；生产仍建议在 **网关/WAF** 叠加防护。
...
- **管理端**：强密码、可选 IP 限制、`ADMIN_TOKEN_EXPIRES_MINUTES`。  
- **TLS**：全站 HTTPS。
